...
router.post('/upload', upload.single('file'), (req, res) => {
    ...
    const csvFilename = `bri_to_import_${baseName}.csv`; // Nama file CSV yang dihasilkan
...
function filterAndFormatRows(data) {
    function formatDate(datetimeStr) {
        ...
        // Pisahkan tanggal dan waktu menggunakan spasi sebagai pemisah
        const [datePart] = datetimeStr.split(' ');
    
        // Kembalikan hanya bagian tanggal
        return datePart;
    }
    ...
    const filteredRows = data.filter(row => {
        const isDate = /^(\d{4})\-(\d{2})\-(\d{2})$/.test(formatDate(row[0]));
        return isDate;
    });
